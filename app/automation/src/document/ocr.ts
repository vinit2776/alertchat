...
export async function extractDocument(
  ...
  const trace = lf?.trace({ name: 'ocr-extract', metadata: { mimeType }, tags: ['ocr'] });
  ...
  const response = await getClient().messages.create({
    ...
    messages: [{
      role:    'user',
      content: [docBlock as any, { type: 'text', text: EXTRACT_PROMPT }],
    }],
  });
  ...
  if (!toolUse) {
    ...
    gen?.end({ output: rawText, level: 'ERROR' });
